...
TableMap.prototype.updateColumnInfo = function() {
  ...
  for (let j = 0; j < this.columnCount; j++) {
    ...
    if  (columnsMetadata[j]) cdata.metadata = columnsMetadata[j]; 
    if (columnSchemas[j].COLUMN_KEY=='PRI') { 
      cdata.isPk = true;
      key.push(j); // push column index
    }
    columns.push(cdata);
  }
  if (key.length) tableMap.key = key; // table has a pk
